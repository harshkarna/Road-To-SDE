package com.Graphs.Striver.BonusAlgo;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class Bridges_Graph_TarjanAlgo {
        ...
        for (Integer it : adj.get(node)) {
            ...
            if (vis[it] == 0) {
                ...
                if (low[it] > tin[node]) {
                    ...
                    criticalConnection.add(it);
                    criticalConnection.add(node);
                    bridges.add(criticalConnection);
                }
            } else {
                low[node] = Math.min(low[node], low[it]);
            }
